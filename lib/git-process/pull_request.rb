...
module GitProc
  ...
  class PullRequest < Process
    ...
    def create_pull_request
      ...
      PullRequest.create_pull_request(self, server_name, remote_name, repo_name, current_branch, base_branch, head_branch, title, description, logger, @user, @password)
    end


    def checkout_pull_request
      PullRequest.checkout_pull_request(self, @pr_number, remote_name, repo_name, @user, @password, logger)
    end
    ...
    class << self
      ...
      def match_remote_to_pr_remote(lib, pr_remote)
        pr_url = lib.expanded_url(nil, pr_remote)
        servers = lib.remote_servers()
        server_urls = servers.collect { |s| {:server_name => s, :url => lib.expanded_url(s)} }
        ...
        pair = server_urls.find do |su|
          ...
          pr_uri = URI.parse(lib.expanded_url(nil, pr_url))
          pr_host = pr_uri.host
          pr_path = pr_uri.path
